...
void test_omp(int M, int *row, int *col, double *val, double *vec, int num_threads) {
	
	double tol = 1e-6;
	
	double *res_serial = (double *) malloc (M * sizeof(double));
	double *res_omp = (double *) malloc (M * sizeof(double));

	spmv_serial(M, row, col, val, vec, res_serial);
	spmv_omp(M, row, col, val, vec, res_omp, num_threads);

	if (compare_results(res_serial, res_omp, M, tol)) {
		printf("passed\n");
	} else {
		printf("failed\n");
	}
...
int main(int argc, char *argv[]) {
	if (argc < 2) {
		fprintf(stderr, "usage: %s -f [martix-market-filename]\n", argv[0]);
		exit(1);
	}
	...
	//test_omp(M, row, col, val, vec, num_threads);
